...
d1 = 1. / 4 # step 1 for [i]
d2 = 1. / 4 # step 1 for [i+1]
d3 = 63. / 32768 # step 2 for [i-3]
d4 = -595. / 32768 # step 2 for [i-2]
d5 = 2687. / 32768 # step 2 for [i-1]
d6 = -8299. / 32768 # step 2 for [i]
d7 = -8299. / 32768 # step 2 for [i+1], not an error
d8 = 2687. / 32768 # step 2 for [i+2]
d9 = -595. / 32768 # step 2 for [i+3]
d10 = 63. / 32768 # step 2 for [i+4]
c1 = 1. # step 1 for [i-1]
c2 = 1. # step 1 for [i]
k = np.sqrt(2.) / 3
DEFAULT_KERNEL = [c1, c2, d1, d2, d3, d4, d5, d6, d7, d8, d9, d10, k]
REV_BIOR_SPLINE_48_KERNEL = DEFAULT_KERNEL


# ----- New vectorized versions -----

def fast_revbiorspline48_1d_op(x, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Reverse-Bior-Spline-4/8 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
    ...
    assert len(kernel) == 13, \
        f'Reverse-Bior-Spline-4/8 kernel must have 13 values, len(kernel)={len(kernel)}, kernel={kernel}'
...
def fast_inv_revbiorspline48_1d_op(x_coefs, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Reverse-Bior-Spline-4/8 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
...
def fast_revbiorspline48_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    # 1. Apply across rows
    x = fast_revbiorspline48_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    # 2. Apply across cols
    x = fast_revbiorspline48_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
...
def fast_inv_revbiorspline48_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    x = fast_inv_revbiorspline48_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
    # 3. Apply inverse transform across rows
    x = fast_inv_revbiorspline48_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    return x
...
if __name__ == '__main__':
    ...
    vis_anz_image, error, _ = test_lifting_scheme(image,
                                                  ...
                                                  forward_2d_op=fast_revbiorspline48_2d_op,
                                                  backward_2d_op=fast_inv_revbiorspline48_2d_op,
                                                  data_format=data_format)

    show_lifting_results(src_image=image, anz_image=vis_anz_image, wavelet_name='Reverse Bior Spline 4/8')
