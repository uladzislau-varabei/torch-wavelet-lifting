...
c2 = 1. / 12 # step 2 for [i-1]
c3 = 4. / 9 # step 2 for [i]
c4 = -1. / 12 # step 2 for [i+1]
d1 = -9. / 8 # step 1 for [i]
d2 = -3. / 8 # step 1 for [i+1]
k = 3. / np.sqrt(2.)
DEFAULT_KERNEL = [c1, c2, c3, c4, d1, d2, k]
BIOR_SPLINE_33_KERNEL = DEFAULT_KERNEL


# ----- New vectorized versions -----

def fast_biorspline33_1d_op(x, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Bior-Spline-3/3 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
    ...
    # o - odd, e - even
    assert len(kernel) == 7, \
        f'Bior-Spline-3/3 kernel must have 7 values, len(kernel)={len(kernel)}, kernel={kernel}'
    c1, c2, c3, c4, d1, d2, k =\
    ...
    x_ev_2 = x_ev_1 + (
        c2 * neg_shift_4d(x_od_1, n_shifts=1, **common_kwargs) +
        c3 * x_od_1 +
        c4 * pos_shift_4d(x_od_1, n_shifts=1, **common_kwargs)
    )
    # Normalization
    if scale_coeffs:
        x_ev_3 = k * x_ev_2 # s
...
def fast_inv_biorspline33_1d_op(x_coefs, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Bior-Spline-3/3 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
...
def fast_biorspline33_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    # 1. Apply across rows
    x = fast_biorspline33_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    # 2. Apply across cols
    x = fast_biorspline33_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
...
def fast_inv_biorspline33_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    x = fast_inv_biorspline33_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
    # 3. Apply inverse transform across rows
    x = fast_inv_biorspline33_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    return x
...
if __name__ == '__main__':
    ...
    vis_anz_image, error, _ = test_lifting_scheme(image,
                                                  ...
                                                  forward_2d_op=fast_biorspline33_2d_op,
                                                  backward_2d_op=fast_inv_biorspline33_2d_op,
                                                  data_format=data_format)

    show_lifting_results(src_image=image, anz_image=vis_anz_image, wavelet_name='Bior Spline 3/3')
