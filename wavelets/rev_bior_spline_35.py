...
d2 = -5. / 288 # step 2 for [i-2]
d3 = 17. / 144 # step 2 for [i-1]
d4 = -4. / 9 # step 2 for [i]
d5 = -17. / 144 # step 2 for [i+1]
d6 = 5. / 288 # step 2 for [i+2]
c1 = 3. / 8 # step 1 for [i-1]
c2 = 9. / 8 # step 1 for [i]
k = np.sqrt(2.) / 3
DEFAULT_KERNEL = [c1, c2, d1, d2, d3, d4, d5, d6, k]
REV_BIOR_SPLINE_35_KERNEL = DEFAULT_KERNEL


# ----- New vectorized versions -----

def fast_revbiorspline35_1d_op(x, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Reverse-Bior-Spline-3/5 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
    ...
    # o - odd, e - even
    assert len(kernel) == 9, \
        f'Reverse-Bior-Spline-3/5 kernel must have 9 values, len(kernel)={len(kernel)}, kernel={kernel}'
    c1, c2, d1, d2, d3, d4, d5, d6, k =\
    ...
    x_od_2 = x_od_1 + (
        d2 * neg_shift_4d(x_ev_1, n_shifts=2, **common_kwargs) +
        d3 * neg_shift_4d(x_ev_1, n_shifts=1, **common_kwargs) +
        d4 * x_ev_1 +
        d5 * pos_shift_4d(x_ev_1, n_shifts=1, **common_kwargs) +
        d6 * pos_shift_4d(x_ev_1, n_shifts=2, **common_kwargs)
    )
    # Normalization
    if scale_coeffs:
        x_ev_2 = k * x_ev_1 # s
...
def fast_inv_revbiorspline35_1d_op(x_coefs, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Reverse-Bior-Spline-3/5 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
...
def fast_revbiorspline35_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    # 1. Apply across rows
    x = fast_revbiorspline35_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    # 2. Apply across cols
    x = fast_revbiorspline35_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
...
def fast_inv_revbiorspline35_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    x = fast_inv_revbiorspline35_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
    # 3. Apply inverse transform across rows
    x = fast_inv_revbiorspline35_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    return x
