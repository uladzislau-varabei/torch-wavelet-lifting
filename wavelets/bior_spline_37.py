...
c2 = -35. / 9216 # step 2 for [i-3]
c3 = 25. / 768 # step 2 for [i-2]
c4 = -421. / 3072 # step 2 for [i-1]
c5 = 4. / 9 # step 2 for [i]
c6 = 421. / 3072 # step 2 for [i+1]
c7 = -25. / 768 # step 2 for [i+2]
c8 = 35 / 9216 # step 2 for [i+3]
d1 = -3. / 8 # step 1 for [i-1]
d2 = -9. / 8 # step 1 for [i]
k = 3. / np.sqrt(2.)
DEFAULT_KERNEL = [c1, c2, c3, c4, c5, c6, c7, c8, d1, d2, k]
BIOR_SPLINE_37_KERNEL = DEFAULT_KERNEL


# ----- New vectorized versions -----

def fast_biorspline37_1d_op(x, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Bior-Spline-3/7 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
    ...
    assert len(kernel) == 11, \
        f'Bior-Spline-3/7 kernel must have 11 values, len(kernel)={len(kernel)}, kernel={kernel}'
    c1, c2, c3, c4, c5, c6, c7, c8, d1, d2, k =\
        kernel[0], kernel[1], kernel[2], kernel[3], kernel[4], kernel[5], \
            kernel[6], kernel[7], kernel[8], kernel[9], kernel[10]
    x_ev_1 = x_ev_0 + (
    ...
    x_ev_2 = x_ev_1 + (
        c2 * neg_shift_4d(x_od_1, n_shifts=3, **common_kwargs) +
        c3 * neg_shift_4d(x_od_1, n_shifts=2, **common_kwargs) +
        c4 * neg_shift_4d(x_od_1, n_shifts=1, **common_kwargs) +
        c5 * x_od_1 +
        c6 * pos_shift_4d(x_od_1, n_shifts=1, **common_kwargs) +
        c7 * pos_shift_4d(x_od_1, n_shifts=2, **common_kwargs) +
        c8 * pos_shift_4d(x_od_1, n_shifts=3, **common_kwargs)
    )
    # Normalization
    if scale_coeffs:
        x_ev_3 = k * x_ev_2 # s
...
def fast_inv_biorspline37_1d_op(x_coefs, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Bior-Spline-3/7 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
...
def fast_biorspline37_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    # 1. Apply across rows
    x = fast_biorspline37_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    # 2. Apply across cols
    x = fast_biorspline37_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
...
def fast_inv_biorspline37_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    x = fast_inv_biorspline37_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
    # 3. Apply inverse transform across rows
    x = fast_inv_biorspline37_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    return x
...
if __name__ == '__main__':
    ...
    vis_anz_image, error, _ = test_lifting_scheme(image,
                                                  ...
                                                  forward_2d_op=fast_biorspline37_2d_op,
                                                  backward_2d_op=fast_inv_biorspline37_2d_op,
                                                  data_format=data_format)

    show_lifting_results(src_image=image, anz_image=vis_anz_image, wavelet_name='Bior Spline 3/7')
