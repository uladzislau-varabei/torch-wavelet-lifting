...
CDF_53_KERNEL = DEFAULT_KERNEL


# ----- New vectorized versions -----

def fast_cdf53_1d_op(x, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'CDF53 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
    ...
    x_ev_1 = x_ev_0 + (
        c1 * x_od_0 +
        c2 * neg_shift_4d(x_od_0, n_shifts=1, **common_kwargs)
    )
    x_od_1 = x_od_0 + (
        d1 * x_ev_1 +
...
def fast_inv_cdf53_1d_op(x_coefs, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Inverse CDF53 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
        'across_cols': across_cols,
        'across_rows': across_rows,
        'data_format': data_format
...
def fast_cdf53_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    # 1. Apply across rows
    x = fast_cdf53_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    # 2. Apply across cols
    x = fast_cdf53_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
...
def fast_inv_cdf53_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    x = fast_inv_cdf53_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
    # 3. Apply inverse transform across rows
    x = fast_inv_cdf53_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    return x
...
if __name__ == '__main__':
    ...
    vis_anz_image, error = test_lifting_scheme(image,
                                               ...
                                               forward_2d_op=fast_cdf53_2d_op,
                                               backward_2d_op=fast_inv_cdf53_2d_op,
                                               data_format=data_format)

    show_lifting_results(src_image=image, anz_image=vis_anz_image, wavelet_name='CDF-5/3 (lossless in JPEG 2000)')
