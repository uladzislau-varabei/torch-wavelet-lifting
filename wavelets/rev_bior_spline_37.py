...
d2 = 35. / 9216 # step 2 for [i-3]
d3 = -25. / 768 # step 2 for [i-2]
d4 = 421. / 3072 # step 2 for [i-1]
d5 = -4. / 9 # step 2 for [i]
d6 = -421. / 3072 # step 2 for [i+1]
d7 = 25. / 768 # step 2 for [i+2]
d8 = -35 / 9216 # step 2 for [i+3]
c1 = 3. / 8 # step 1 for [i-1]
c2 = 9. / 8 # step 1 for [i]
k = np.sqrt(2.) / 3
DEFAULT_KERNEL = [c1, c2, d1, d2, d3, d4, d5, d6, d7, d8, k]
REV_BIOR_SPLINE_37_KERNEL = DEFAULT_KERNEL


# ----- New vectorized versions -----

def fast_revbiorspline37_1d_op(x, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Reverse-Bior-Spline-3/7 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
    ...
    d1, d2, d3, d4, d5, d6, d7, d8 =\
        kernel[2], kernel[3], kernel[4], kernel[5], \
            kernel[6], kernel[7], kernel[8], kernel[9]
    x_od_1 = x_od_0 + (
    ...
    x_od_2 = x_od_1 + (
        d2 * neg_shift_4d(x_ev_1, n_shifts=3, **common_kwargs) +
        d3 * neg_shift_4d(x_ev_1, n_shifts=2, **common_kwargs) +
        d4 * neg_shift_4d(x_ev_1, n_shifts=1, **common_kwargs) +
        d5 * x_ev_1 +
        d6 * pos_shift_4d(x_ev_1, n_shifts=1, **common_kwargs) +
        d7 * pos_shift_4d(x_ev_1, n_shifts=2, **common_kwargs) +
        d8 * pos_shift_4d(x_ev_1, n_shifts=3, **common_kwargs)
    )
    # Normalization
    if scale_coeffs:
        x_ev_2 = k * x_ev_1 # s
...
def fast_inv_revbiorspline37_1d_op(x_coefs, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Reverse-Bior-Spline-3/7 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
...
def fast_revbiorspline37_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    # 2. Apply across cols
    x = fast_revbiorspline37_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
...
def fast_inv_revbiorspline37_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    x = fast_inv_revbiorspline37_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
    # 3. Apply inverse transform across rows
    x = fast_inv_revbiorspline37_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    return x
...
if __name__ == '__main__':
    ...
    vis_anz_image, error = test_lifting_scheme(image,
                                               ...
                                               forward_2d_op=fast_revbiorspline37_2d_op,
                                               backward_2d_op=fast_inv_revbiorspline37_2d_op,
                                               data_format=data_format)

    show_lifting_results(src_image=image, anz_image=vis_anz_image, wavelet_name='Reverse Bior Spline 3/7')
