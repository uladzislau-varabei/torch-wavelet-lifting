...
d2 = 1. / 12 # step 2 for [i-1]
d3 = -4. / 9 # step 2 for [i]
d4 = -1. / 12 # step 2 for [i+1]
c1 = 3. / 8 # step 1 for [i-1]
c2 = 9. / 8 # step 1 for [i]
k = np.sqrt(2.) / 3
DEFAULT_KERNEL = [c1, c2, d1, d2, d3, d4, k]
REV_BIOR_SPLINE_33_KERNEL = DEFAULT_KERNEL


# ----- New vectorized versions -----

def fast_revbiorspline33_1d_op(x, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Reverse-Bior-Spline-3/3 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
    ...
    # o - odd, e - even
    assert len(kernel) == 7, \
        f'Reverse-Bior-Spline-3/3 kernel must have 7 values, len(kernel)={len(kernel)}, kernel={kernel}'
    c1, c2, d1, d2, d3, d4, k =\
    ...
    x_od_2 = x_od_1 + (
        d2 * neg_shift_4d(x_ev_1, n_shifts=1, **common_kwargs) +
        d3 * x_ev_1 +
...
def fast_inv_revbiorspline33_1d_op(x_coefs, kernel, scale_coeffs, across_cols=False, across_rows=False, data_format=DEFAULT_DATA_FORMAT):
    # x shape: [N, C, H, W] or [N, H, W, C]
    assert not(across_cols and across_rows) and (across_cols or across_rows), \
        f'Reverse-Bior-Spline-3/3 1d op: across_cols = {across_cols}, across_rows = {across_rows}'
    common_kwargs = {
...
def fast_revbiorspline33_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    # 1. Apply across rows
    x = fast_revbiorspline33_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    # 2. Apply across cols
    x = fast_revbiorspline33_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
...
def fast_inv_revbiorspline33_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    x = fast_inv_revbiorspline33_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=True, across_rows=False, data_format=data_format)
    # 3. Apply inverse transform across rows
    x = fast_inv_revbiorspline33_1d_op(x, kernel, scale_coeffs=scale_1d_coeffs, across_cols=False, across_rows=True, data_format=data_format)
    return x
...
if __name__ == '__main__':
    ...
    vis_anz_image, error, _ = test_lifting_scheme(image,
                                                  ...
                                                  forward_2d_op=fast_revbiorspline33_2d_op,
                                                  backward_2d_op=fast_inv_revbiorspline33_2d_op,
                                                  data_format=data_format)

    show_lifting_results(src_image=image, anz_image=vis_anz_image, wavelet_name='Reverse Bior Spline 3/3')
